...
<script>
    ...
    <?php if (!$coupon): ?>
        document.getElementById('applyCouponCo')?.addEventListener('click', function() {
            ...
            if (!code) return;
            var btn = this,
                msg = document.getElementById('couponMsgCo');
            btn.disabled = true;
            btn.textContent = '...';
            msg.innerHTML = '';
            $.post('coupon_handler.php', {
                action: 'validate',
                code,
                cart_subtotal: checkoutSubtotal.toFixed(2)
            }, function(raw) {
                var data = parseReply(raw);
                btn.disabled = false;
    ...
    function proceedToPayment() {
        if (!selectedAddrId) {
            showCoToast('Please select a delivery address.', false);
            return;
        }
