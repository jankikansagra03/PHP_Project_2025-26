...
if (!$res || mysqli_num_rows($res) === 0) { header('Location: shop.php'); exit; }
...
$reviews      = [];
$total_rating = 0;
while ($r = mysqli_fetch_assoc($rev_res)) { $reviews[] = $r; $total_rating += $r['rating']; }
...
<style>
/* ── Gallery ── */
.pd-gallery-wrap { position: relative; display: flex; flex-direction: row; gap: 12px; }
.pd-thumb-strip {
    display: flex;
...
.pd-thumb-strip::-webkit-scrollbar { width: 4px; }
.pd-thumb-strip::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
.pd-thumb {
...
.pd-thumb img { width: 85%; height: 85%; object-fit: contain; mix-blend-mode: multiply; }
.pd-thumb.active { border-color: var(--theme-primary, #1f7a8c); background: #fff; box-shadow: 0 0 0 3px rgba(31,122,140,.15); }
.pd-thumb:hover:not(.active) { border-color: #94a3b8; background: #fff; }
.pd-main-area { flex: 1; min-width: 0; }
.pd-main-img {
...
.pd-old-price { text-decoration: line-through; color: #94a3b8; }
.pd-discount-badge {
...
/* ── Status Badges ── */
.pd-status-row { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 1.25rem; }
.pd-status-pill {
...
.pill-category { background: rgba(31,122,140,.1); color: var(--theme-primary, #1f7a8c); }
.pill-instock  { background: #dcfce7; color: #15803d; }
.pill-outstock { background: #fee2e2; color: #b91c1c; }
.pill-incart   { background: #dbeafe; color: #1d4ed8; }
.pill-inwl     { background: #fce7f3; color: #9d174d; }

/* ── Qty Stepper ── */
.pd-qty-stepper { display: flex; align-items: center; gap: 0; border-radius: 12px; overflow: hidden; border: 2px solid #e2e8f0; width: fit-content; }
...
.pd-qty-stepper button:hover { background: rgba(31,122,140,.1); }
.pd-qty-stepper input {
    width: 52px; border: none; text-align: center;
    font-weight: 700; font-size: 1rem; background: none;
...
.pd-btn-cart:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(31,122,140,.4); }
.pd-btn-cart:active { transform: translateY(0); }
.pd-btn-cart.in-cart { background: linear-gradient(135deg, #16a34a, #15803d); box-shadow: 0 4px 15px rgba(22,163,74,.3); }
...
.pd-btn-wl:hover { border-color: #f43f5e; color: #f43f5e; background: #fef2f2; transform: scale(1.08); }
.pd-btn-wl.in-wl { border-color: #f43f5e; background: #f43f5e; color: #fff; }
.pd-btn-wl.in-wl:hover { background: #e11d48; border-color: #e11d48; }

/* ── Trust badges ── */
.pd-trust-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 1.5rem; }
.pd-trust-item {
...
.pd-trust-item i { font-size: 1.4rem; margin-bottom: 6px; display: block; }
.pd-trust-item strong { font-size: .8rem; display: block; color: #374151; }
.pd-trust-item small { font-size: .7rem; color: #9ca3af; }

/* ── Tabs ── */
.pd-tabs { display: flex; gap: 0; background: #f1f5f9; border-radius: 14px; padding: 5px; margin-bottom: 1.5rem; }
.pd-tab-btn {
    flex: 1; padding: 10px; font-weight: 600; font-size: .88rem;
    border: none; background: none; border-radius: 10px;
    cursor: pointer; transition: all .2s; color: #64748b;
    position: relative;
}
.pd-tab-btn.active { background: #fff; color: var(--theme-primary, #1f7a8c); box-shadow: 0 2px 8px rgba(0,0,0,.1); }
...
.related-card:hover { transform: translateY(-5px); box-shadow: 0 12px 40px rgba(0,0,0,.12); }
.related-img { aspect-ratio: 1/1; background: #f8f9fa; display: flex; align-items: center; justify-content: center; overflow: hidden; }
.related-img img { width: 80%; height: 80%; object-fit: contain; mix-blend-mode: multiply; transition: transform .3s; }
.related-card:hover .related-img img { transform: scale(1.07); }

/* ── Toast ── */
#pdToast { min-width: 260px; }
...
.pd-zoom-overlay.show { display: flex; }
.pd-zoom-overlay img { max-width: 90vw; max-height: 90vh; object-fit: contain; border-radius: 12px; }
</style>
        ...
        <div class="col-lg-6">
            ...
            <div class="d-flex align-items-center gap-2 mb-3">
                <div class="d-flex gap-1">
                    <?php
                    $full = floor($avg_rating); $half = ($avg_rating - $full) >= 0.5 ? 1 : 0; $empty = 5 - $full - $half;
                    ...
                    <div style="background:#fff;border-radius:20px;border:1px solid #e2e8f0;padding:1.5rem;margin-bottom:1.25rem;">
                        ...
                        <div class="text-center mb-3">
                            ...
                            <div class="d-flex gap-1 justify-content-center my-2">
                                <?php
                                $full2 = floor($avg_rating); $half2 = ($avg_rating - $full2) >= 0.5 ? 1 : 0; $emp2 = 5 - $full2 - $half2;
...
document.getElementById('addToCartBtn')?.addEventListener('click', function() {
    var btn = this, pid = btn.dataset.productId, qty = parseInt(qtyInput?.value || 1);
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm" style="width:16px;height:16px;"></span>&nbsp; Adding...';
    $.post('cart_handler.php', {action:'add', product_id:pid, quantity:qty}, function(data) {
        btn.disabled = false;
        if (data.redirect) { window.location.href = data.redirect; return; }
        showPdToast(data.message, data.success);
        if (data.success) {
            updateCartBadge(data.cart_count);
            btn.classList.add('in-cart');
            btn.innerHTML = '<i class="fas fa-check"></i> Added to Cart';
            var badge = document.getElementById('inCartBadge');
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-shopping-cart"></i> Add to Cart';
        showPdToast('Something went wrong. Please try again.', false);
    });
});

// ── Wishlist Toggle ──────────────────────────────────
document.getElementById('wishlistBtn')?.addEventListener('click', function() {
    var btn = this, pid = btn.dataset.productId, inWl = btn.dataset.inWishlist === '1';
    btn.disabled = true;
    var origHtml = btn.innerHTML;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm" style="width:16px;height:16px;border-width:2px;"></span>';
    $.post('wishlist_handler.php', {action: inWl ? 'remove' : 'add', product_id: pid}, function(data) {
        btn.disabled = false;
        if (data.redirect) { window.location.href = data.redirect; return; }
        showPdToast(data.message, data.success);
        if (data.success) {
            var inWlBadge = document.getElementById('inWlBadge');
            if (inWl) {
                btn.dataset.inWishlist = '0';
                btn.classList.remove('in-wl');
                btn.innerHTML = '<i class="far fa-heart"></i>';
                btn.title = 'Save to Wishlist';
                if (inWlBadge) inWlBadge.style.display = 'none';
                ...
                if (inWlBadge) inWlBadge.style.display = '';
                updateWishlistBadge(+1);
            }
        } else {
        ...
        btn.disabled = false;
        btn.innerHTML = origHtml;
        showPdToast('Something went wrong.', false);
    });
});
...
document.getElementById('submitReviewBtn')?.addEventListener('click', function() {
    ...
    if (!rating || rating < 1)        { showMsg('⭐ Please select a star rating.', false); return; }
    if (!title || title.length < 3)   { showMsg('Please enter a review title (min 3 characters).', false); return; }
    if (!review || review.length < 10){ showMsg('Review must be at least 10 characters.', false); return; }

    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm" style="width:16px;height:16px;"></span>&nbsp;Submitting...';
    msgBox.innerHTML = '';

    $.post('review_handler.php', { product_id: <?= $product_id ?>, rating:rating, title:title, review:review },
    function(data) {
        btn.disabled = false;
        ...
        if (data.success) {
            ...
            setTimeout(function() {
                ...
                var formCard = document.querySelector('[id="submitReviewBtn"]')?.closest('div[style*="border:1px solid #e2e8f0"]');
                if (formCard) {
                    formCard.innerHTML = '<div style="text-align:center;padding:1.5rem;">' +
                        '<div style="font-size:2.5rem;margin-bottom:.5rem;">🎉</div>' +
                        '<h6 style="font-weight:800;color:#15803d;">Thank you!</h6>' +
                        '<p style="color:#64748b;font-size:.82rem;margin:0;">Your review has been published.</p></div>';
                }
                // Inject new review card at top of list
                var colors = ['#3b82f6','#8b5cf6','#f59e0b','#10b981','#ef4444','#06b6d4','#f97316','#ec4899'];
                var color  = colors[Math.floor(Math.random() * colors.length)];
                var stars  = '';
                for (var i = 1; i <= 5; i++) {
                    stars += '<i class="' + (i <= data.rating ? 'fas' : 'far') + ' fa-star text-warning" style="font-size:.78rem;"></i>';
                }
                var today = new Date().toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'});
                var newCard = '<div class="review-card" style="border-left:3px solid '+color+';">' +
                    '<div class="d-flex align-items-start gap-3 mb-2">' +
                    '<div style="width:44px;height:44px;border-radius:50%;background:'+color+';display:flex;align-items:center;justify-content:center;font-weight:800;font-size:1rem;color:#fff;flex-shrink:0;">'+data.initial+'</div>' +
                    '<div class="flex-grow-1"><div class="d-flex justify-content-between flex-wrap gap-1">' +
                    '<div><span style="font-weight:700;color:#0f172a;font-size:.9rem;">'+data.user_name+'</span>' +
                    '<span style="background:#dbeafe;color:#1d4ed8;font-size:.6rem;font-weight:700;padding:2px 7px;border-radius:20px;margin-left:6px;">Your Review</span></div>' +
                    '<div>'+stars+'</div></div>' +
                    '<div style="color:#94a3b8;font-size:.72rem;"><i class="fas fa-calendar-alt me-1" style="font-size:.65rem;"></i>'+today+'</div></div></div>' +
                    '<h6 style="font-weight:700;color:#1e293b;font-size:.88rem;margin-bottom:5px;">'+data.title+'</h6>' +
                    '<p style="color:#64748b;font-size:.83rem;line-height:1.75;margin:0;">'+data.review.replace(/\n/g,'<br>')+'</p></div>';
                var list = document.getElementById('reviewCardsList');
                if (list) {
                    var empty = list.querySelector('div[style*="dashed"]');
