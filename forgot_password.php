<?php
// Start session and include helpers for DB access and email dispatch.
@session_start();
include 'mailer.php';
...
// Process forgot-password form submission and generate OTP state.
if (isset($_POST['send_otp'])) {
    $email = trim($_POST['email']);

    // Check existing reset-token row to track attempts and throttle abuse.

    $q = "select * from password_token where email='$email'";
    ...
    if ($count > 0) {
        // Existing token path: rotate OTP if max attempts are not reached.
        ...
        if ($attempts >= 3) {
            ...
            $update_query = "UPDATE password_token SET otp = $new_otp, created_at = '$email_time', expires_at = '$expiry_time', otp_attempts = $attempts, last_resend = CURRENT_TIMESTAMP WHERE email = '$email'";

            if (mysqli_query($con, $update_query)) {
                // Mark DB success so email can be sent in the next step.
                $db_success = true;
                $send_email = true;
        ...
        // First token path: create token row for this email.
        $attempts = 1;
        $new_otp = rand(100000, 999999);
        $email_time = date("Y-m-d H:i:s");
        $expiry_time = date("Y-m-d H:i:s", strtotime('+2 minutes'));

        $insert_query = "INSERT INTO password_token (email, otp, created_at, expires_at, otp_attempts) VALUES ('$email', $new_otp, '$email_time', '$expiry_time', $attempts)";
    ...
    if ($db_success && $send_email) {
        ...
        // Catch-all DB failure path when no earlier redirect is set.
        setcookie('error', 'Failed to update the database. Please try again.', time() + 5, "/");
        $redirect_url = "forgot_password.php";
    }

    // Redirect once final destination is selected.
    if ($redirect_url) {
...
<script>
    // Validate entered email via AJAX before user requests OTP.
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (responseTrimmed == 'false') {
                        // Block submission when email is not registered.
                        ...
                        // Allow submission when email exists.
                        $('#email_error').text('').hide();
                        $('#email').removeClass('is-invalid');
                        $('#sendOtpButton').prop('disabled', false);
                    } else {
                        // Defensive fallback for unexpected responses.
