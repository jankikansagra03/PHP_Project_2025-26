...
if (isset($_POST['action'])) {
    ...
    if ($action === 'create') {
        ...
        if ($userEmail === '' || $productId <= 0) {
            setcookie('error', 'Please enter valid cart details.', time() + 5, '/');
        ...
        if (mysqli_stmt_execute($stmt)) setcookie('success', 'Cart item created.', time() + 5, '/');
        else setcookie('error', 'Failed to create cart item.', time() + 5, '/');
...
$perPage = 10;
$where = $search !== '' ? ' WHERE c.user_email LIKE ? OR p.name LIKE ?' : '';
$like = '%' . $search . '%';

$countSql = 'SELECT COUNT(*) AS total FROM cart c LEFT JOIN products p ON p.id = c.product_id' . $where;
...
$listSql = 'SELECT c.*, p.name AS product_name, p.image AS product_image, p.price FROM cart c LEFT JOIN products p ON p.id = c.product_id' . $where . ' ORDER BY c.id DESC LIMIT ?, ?';
...
<div class="page-card">
    <div class="products-header d-flex flex-wrap align-items-center justify-content-between gap-2">
        ...
        <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addCartModal"><i class="fas fa-plus me-1"></i>Add Cart Item</button>
    </div>

    <div class="products-body">
        <form method="get" class="mb-3" novalidate>
            <input type="text" id="searchInput" class="form-control" name="search" placeholder="Search by user email or product..." value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>">
        </form>
                ...
                <tbody>
                    <?php if ($result && mysqli_num_rows($result) > 0): ?>
                        <?php while ($row = mysqli_fetch_assoc($result)): ?>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center gap-2">
                                        <?php if (!empty($row['product_image'])): ?><img src="<?= htmlspecialchars((string) $row['product_image'], ENT_QUOTES, 'UTF-8') ?>" class="small-preview border" alt="product"><?php endif; ?>
                                        <div>
                                ...
                                <td>
                                    <div class="products-actions d-flex gap-1"><button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#editCartModal<?= (int) $row['id'] ?>" title="Edit" aria-label="Edit"><i class="fas fa-pen"></i></button><button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteCartModal<?= (int) $row['id'] ?>" title="Delete" aria-label="Delete"><i class="fas fa-trash"></i></button></div>
                                </td>
                            </tr>

                            <div class="modal fade" id="editCartModal<?= (int) $row['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form method="post" novalidate>
                                            <div class="modal-header">
                                                <h5 class="modal-title">Edit Cart Item</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body"><input type="hidden" name="action" value="update"><input type="hidden" name="id" value="<?= (int) $row['id'] ?>"><input type="hidden" name="return_search" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="return_page" value="<?= (int) $page ?>">
                                                ...
                                                <div class="mb-1"><label class="form-label">Quantity</label><input type="number" class="form-control" min="1" name="quantity" value="<?= (int) ($row['quantity'] ?? 1) ?>" required></div>
                                            </div>
                            ...
                            <div class="modal fade" id="deleteCartModal<?= (int) $row['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <form method="post" novalidate>
                                            <div class="modal-header">
                                                <h5 class="modal-title">Delete Cart Item</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                            </div>
                                            <div class="modal-body">
                                                <p>Delete this cart item permanently?</p><input type="hidden" name="action" value="delete"><input type="hidden" name="id" value="<?= (int) $row['id'] ?>"><input type="hidden" name="return_search" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="return_page" value="<?= (int) $page ?>">
                                            </div>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">No cart items found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <nav class="products-pagination" aria-label="Cart pagination">
...
<div class="modal fade" id="addCartModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" novalidate>
                <div class="modal-header">
                    <h5 class="modal-title">Add Cart Item</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body"><input type="hidden" name="action" value="create"><input type="hidden" name="return_search" value="<?= htmlspecialchars($search, ENT_QUOTES, 'UTF-8') ?>"><input type="hidden" name="return_page" value="<?= (int) $page ?>">
                    ...
                    <div class="mb-1"><label class="form-label">Quantity</label><input type="number" class="form-control" min="1" name="quantity" value="1" required></div>
                </div>
