        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            ...
            <div class="d-flex gap-2 flex-wrap">
                ...
                foreach ($tabs as $val => $label):
                    $active = $filter === $val ? 'btn-gradient' : 'btn-outline-gradient';
                ?>
...
<script>
    function viewOrderDetails(orderId) {
        var modal = new bootstrap.Modal($('#orderDetailsModal').get(0));
        $('#detailModalBody').html('<div class="text-center py-5"><i class="fas fa-spinner fa-spin fa-2x text-muted"></i></div>');
        $('#detailModalOrderNum').text('');
        modal.show();

        $.get('order_detail_ajax.php', {
            order_id: orderId
        }, function(htmlData) {
            // If the server returns an alert-danger string, it means there was an error
            $('#detailModalBody').html(htmlData);

            // Extract order number from the hidden container
            var $container = $('#modalDataContainer');
            if ($container.length) {
                $('#detailModalOrderNum').text($container.attr('data-order-number'));
                $('#detailModalTitle').text('Order Details');
            } else {
                $('#detailModalTitle').text('Error');
            }
        }, 'text').fail(function() {
            $('#detailModalBody').html('<div class="alert alert-danger">Failed to load order details.</div>');
        });
    }
</script>
